...
import { Mail, Phone, Handshake, Tag, CalendarArrowUp } from "lucide-react"
    ...
    <Card className="bg-white shadow-md rounded-lg w-full overflow-hidden">
      <CardHeader className="p-6">
        <div className="flex md:flex-row flex-col md:justify-between md:items-center gap-3">
          <div>
            <CardTitle className="mb-2 font-bold text-3xl">{contact.name}</CardTitle>
      ...
      <CardContent className="px-6 py-4">
        <div className={`gap-6 grid grid-cols-1 ${contact.client ? 'xl:grid-cols-4': 'xl:grid-cols-3'}`} >
          ...
          <div className="flex xl:justify-center items-center space-x-3">
            <CalendarArrowUp className="w-6 h-6 text-gray-400" />
            <span className="text-gray-600 text-lg">{formatDate(contact.createdAt)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
